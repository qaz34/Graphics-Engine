...
namespace aie {
	...
	Texture::Texture(const char * filename, TextureType texturetype, std::string name)
		: m_filename("none"),
	...
	Texture::Texture(unsigned int width, unsigned int height, Format format, std::string name, TextureType texturetype, unsigned char* pixels)
		: m_filename("none"),
		m_width(width),
		m_name(name),
		m_height(height),
	...
	void Texture::SetToHandle(unsigned int handle, unsigned int width, unsigned int height)
	{
		m_width = width;
		m_height = height;
		m_glHandle = handle;
